...
namespace TabletopTweaks.Core.Config {
    public class Blueprints : IUpdatableSettings {
            ...
            if (!NewBlueprints.TryGetValue(name, out Id)) {
                if (!DerivedBlueprints.TryGetValue(name, out Id)) {
                    ...
                    if (!AutoGenerated.TryGetValue(name, out Id)) {
                        ...
                        TTTContext.Logger.LogVerbose($"Generated new GUID: {name} - {Id}");
                    } else {
                        TTTContext.Logger.LogVerbose($"WARNING: GUID: {name} - {Id} is autogenerated");
                    }
#endif
                }
            }
            if (Id == null) { TTTContext.Logger.LogError($"ERROR: GUID for {name} not found"); }
            UsedGUIDs[name] = Id;
        ...
        public BlueprintGuid GetDerivedMaster(string name) {
            ...
            if (!DerivedBlueprintMasters.TryGetValue(name, out Id)) {
                ...
                if (!AutoGenerated.TryGetValue(name, out Id)) {
                    ...
                    AutoGenerated.Add(name, Id);
                    TTTContext.Logger.LogVerbose($"WARNING: MASTER GUID: {name} - {Id} is autogenerated");
                }
#endif
            }
            if (Id == null) { TTTContext.Logger.LogError($"ERROR: MASTER GUID {name} not found"); }
            UsedGUIDs[name] = Id;
            return new BlueprintGuid(Id);
        }
        ...
        static class AutoGUID_Log_Patch {
            ...
            static void Postfix() {
                GenerateUnused();
                Main.TTTContext.SaveSettings("Blueprints.json", Main.TTTContext.Blueprints);
            }
            static void GenerateUnused() {
                Main.TTTContext.Blueprints.AutoGenerated.ForEach(entry => {
                    if (!Main.TTTContext.Blueprints.UsedGUIDs.ContainsKey(entry.Key)) {
                        Main.TTTContext.Blueprints.UnusedGUIDs[entry.Key] = entry.Value;
                    }
                });
                Main.TTTContext.Blueprints.NewBlueprints.ForEach(entry => {
                    if (!Main.TTTContext.Blueprints.UsedGUIDs.ContainsKey(entry.Key)) {
                        Main.TTTContext.Blueprints.UnusedGUIDs[entry.Key] = entry.Value;
                    }
                });
